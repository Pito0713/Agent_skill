...
# ─────────────────────────────────────────
# audit_personal_paths：注入完成後檢查 CLAUDE.md 是否含個人絕對路徑
# 公開 repo 會洩漏使用者名、且路徑對其他協作者全部斷鏈——制度/skill 引用一律 ~ 形式
# （防的是其他 session 手動塞入的內容；INJECT_BLOCK 本身只用 @~/.claude/...）
# ─────────────────────────────────────────
audit_personal_paths() {
  local hits
  hits=$(grep -nE 'file:///|/Users/[A-Za-z0-9._-]+' "$CLAUDE_MD" 2>/dev/null || true)
  if [[ -n "$hits" ]]; then
    echo ""
    echo -e "${YELLOW}⚠️  CLAUDE.md 含個人絕對路徑（若為公開 repo：洩漏使用者名 + 對他人斷鏈）：${NC}"
    echo "$hits" | head -10
    echo "   → 請改為 ~ 開頭路徑（@~/.claude/...）或手動移除該段"
  fi
}
...
install_hook() {
  ...
  mkdir -p "$(dirname "$HOOK_DEST")"   # 部分 repo 沒有 .git/hooks/ 目錄，cp 會直接失敗
...
# ─────────────────────────────────────────
# install_stop_hook：在專案 .claude/settings.json 掛 Claude Code Stop hook（收工守門）
# 設計：governance/enforcement-layers.md §4。settings.json 只放「指向正本」的指令，
# 不複製腳本到下游——避免下游副本漂移（lessons 2026-07-07 教訓）。
# 只動 hooks.Stop；PreToolUse 層由使用者既有全域 hook 承擔，一律不碰。
# ─────────────────────────────────────────
install_stop_hook() {
  local hook_script="$HOME/Agent_skill/hooks/stop-handoff-check.sh"
  local settings="$PROJECT_DIR/.claude/settings.json"

  if [[ ! -f "$hook_script" ]]; then
    echo "⚠️  找不到 $hook_script，略過 Stop hook 安裝"
    return
  fi

  if [[ -f "$settings" ]] && grep -q "stop-handoff-check.sh" "$settings"; then
    echo "✅ Stop hook 已存在（$settings），略過"
    return
  fi

  if ! command -v python3 >/dev/null 2>&1; then
    echo "⚠️  缺 python3 無法安全合併 settings.json，略過 Stop hook 安裝"
    return
  fi

  echo ""
  read -r -p "是否掛載收工守門 Stop hook（交接檔未寫會擋一次收工）？(y/N) " stop_confirm
  [[ ! "$stop_confirm" =~ ^[Yy]$ ]] && echo "略過 Stop hook 安裝。" && return
...
entry = {"hooks": [{"type": "command",
                    "command": "$HOME/Agent_skill/hooks/stop-handoff-check.sh"}]}
try:
    with open(path) as f:
        data = json.load(f)
    ...
    print(f"⚠️  {path} 不是合法 JSON，不動它——請手動處理", file=sys.stderr)
    sys.exit(1)

data.setdefault("hooks", {}).setdefault("Stop", []).append(entry)
with open(path, "w") as f:
    json.dump(data, f, ensure_ascii=False, indent=2)
    f.write("\n")
PYEOF
  if [[ $? -eq 0 ]]; then
    echo -e "${GREEN}✅ 已掛載 Stop hook（指向正本，不複製腳本）${NC}"
    echo "   路徑：$settings"
  fi
}
...
EOF
  ...
  echo "   路徑：$CLAUDE_MD"
  echo ""
  echo -e "${CYAN}📌 常駐載入已設定（6 個 skills）${NC}"
...
PYEOF
  ...
  echo "   路徑：$CLAUDE_MD"
  echo ""
  echo -e "${CYAN}📌 常駐載入已更新（6 個 skills）${NC}"
  install_hook
  install_stop_hook
  audit_personal_paths
  exit 0
