...
│   ├── judgment-rubrics.md            # 判斷力外化（R1-R6，附正反例）
...
| v5.3 | 2026-08-04 | 棄用專案退場 + hook 部署漂移修復：`scan-downstream.sh` 已知下游從 7 個收斂為 3 個（4 個棄用專案其中 3 個目錄已不存在），TODO 舊 `@` 路徑遷移項經逐條 resolve 實測後刪除；修復 ADR-016 兩項未落地成果——補裝 4 份 pre-commit wrapper（實際安裝數原為 0，文件卻宣稱 6 份）、刪除 `shopee` 殘留 post-commit hook（ADR-014/015 禁止的 latest.md 自動寫入路徑）；`enforcement-layers.md` 部署清單改附現跑指令 |
| v5.4 | 2026-08-04 | 交接檔納入版本控制（ADR-017／TODO Phase D）：`~/.agent-sessions` git 化並附 README 三紅線（不加 remote／不裝 pre-commit-audit／commit 不掛 hook），`handoff` skill 寫完 latest.md 立刻 commit、標頭新增 `> 寫入者：` 必填欄，`maintenance-protocol §6` 與 `handoff-verifier` 同步；解除五漏洞中的 #3 覆蓋即永久丟失與 #4 無寫入者身分，並發覆蓋（#1/#2/#5）仍待 Phase A–C |
| v5.4.1 | 2026-08-04 | 文件清理：刪除 `memory/project-context.md` 的 17 處通用模板空占位（架構概覽／技術選型／禁止改動／外部依賴／環境設定／版本歷史，制度倉庫永遠不會有內容，卻被連續 4 份 handoff 誤掃成待辦），改寫為一段「為什麼沒有這些欄位 + 不要加回來」；TODO 依自身慣例收掉已完成的 Phase D 細項；`letter-to-future-sessions` 未完成事項同步 |
| v5.5 | 2026-08-07 | 常駐 skill description 瘦身：38 份 SKILL.md 的 frontmatter description 由 11,592 → 4,641 字元（−60%，每 session 常駐約省 5k tokens），刪除的是對選擇無用的內部流程清單（body 全數保留、diff 未動 body），觸發清單改與 `skills/llms.txt` 正本逐字對齊（同步檢查 0 不一致）；`llms.txt` 歧義樹補 E 選項（模糊詞「優化／完善」加入 unknown-matrix-navigation 出口，補既有缺口）；`academic-mentor` 觸發詞排除條款由「神經科學優先 mentor-neuro」擴為「神經／社會／自然／技術科學優先走對應 mentor-*」，修掉與 mentor-society 的路由撞車。驗證：代號化 A/B 冷啟動路由測試（38 能力匿名化、30 句語句、兩個互不知情 subagent）新舊一致 28/30，差異兩處皆非退步 |
| v5.6 | 2026-08-08 | skill 使用統計 Phase 1（ADR-018）：新增唯讀掃描器 `bin/skill-usage.py` + `bin/lib_skill_usage.py`，事後挖 Claude transcript 與 Codex rollout（agy 為 sqlite blob，明示不涵蓋），三指標 `inv`／`read`／`edit` **永不合併**——「讀到」不等於「選用」；workdir 缺失或落在本 repo 內的讀取一律進 `?` 不猜；`skills/index.json` 新增 `lifecycle` 欄，validator 改為只以四個路由欄位與 `llms.txt` 比對。計畫書經 Codex 冷啟動對抗式審查（6 BLOCKER 全採納，2 處駁回記於 §9）。**首版只輸出觀測值，不分層不退場**——實算顯示現有樣本會把半數 skill 含 `deploy-prep`／`security-review` 丟進退場評估 |
| v5.6.1 | 2026-08-16／17 | 新增 lifestyle 分類與 cooking-flow（ADR-021）：開 `skills/lifestyle/` 第六個分類（既有五類都不含非技術的生活類 skill），cooking-flow 為協調器 + 三模式（食譜產生／冰箱反查／週餐規劃），共用同一組 references 與 script 故不拆三個 skill；**數字全部走 `scripts/` 計算**（份量縮放、單位換算、烤溫烤模、採購加總），LLM 只寫敘述與挑菜，職責切分複用 tw-stock-tracker；換算係數正本只放 `scripts/units_table.py`，`references/units.md` 一個係數都不重抄（兩處各抄一份正是靜默漂移的來源）。**同批推翻初版決策：`cli-delegate` 委派層由 agy 全面換成 codex**——安全邊界從設定檔（`excludeTools`）改為逐次旗標 `-s read-only`，Step 2 的 settings.json bootstrap 整段刪除（−38 行），網搜走 `-c tools.web_search=true`（`codex exec` 不吃 `--search`，傳入 exit 2）；範圍限唯讀委派，agy 仍是第三個 harness，tech-lead-mode Path B 不動。另移除 cooking-flow 食安層（使用者取捨，`rules/security.md` 底線未放寬）；frontmatter description 640 → 317 bytes，**選擇壓縮而非申請 waiver**。驗證：validator PASS 39、gen-skill-frontmatter 39/39、pre-commit-audit exit 0，兩支 script 實跑含四條錯誤路徑 |
| v5.6.2 | 2026-08-18 | cooking-flow 模式 A 步驟改五欄表格（步驟／時間／做什麼／✅ 完成訊號／⚠️ 注意）：使用者實測第一份食譜後的回饋——邊做邊看時散文式編號步驟掃視太慢，文末的「關鍵控制點」要等做完才讀到，等於沒作用。**表格逼出一條約束：每格一句話**，`做什麼` 撐不下一句就代表這步該拆成兩步，不准塞長句繞過；散文格式下可以寫三句混過去的步驟，進表格會直接暴露它其實是兩件事。關鍵控制點不再自成一節，火候／順序／時機一律下沉到所屬步驟的 `⚠️` 格，做到那一步才讀得到，只有跨步驟原則留在表格上方 `⚠️ 全程`；`🔺` 標最容易失敗的那一步、整份只標一個，且該列 `⚠️` 必須寫失敗後果不能只寫「小心」。「想調整的話」正式化為「＋ 加分調整」並標明選配區，與必做步驟切乾淨 |
