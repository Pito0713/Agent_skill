...
description: |
  實作任何功能的強制四階段流程（理解→計畫→實作→驗證）與 commit 前 checklist。 觸發：每次 session 自動生效（常駐）
metadata:
...
若符合 `skills/engineering/tech-lead-mode/SKILL.md` 啟用條件（預估 >3 檔案 / 曾卡關 / 高風險 / 易 scope creep），詢問使用者是否切換為 tech-lead-mode 執行 Phase 3（工單化 + 委派 executor + close gate），取代直接實作。不符合條件則略過，直接進入 Phase 3。
