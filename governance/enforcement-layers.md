...
| # | 產物 | 型 | 內容 | 誤報風險 |
|---|------|-----|------|---------|
| 1 | `hooks/stop-handoff-check.sh` | L2 注入 | Stop hook：本 session 有 repo 寫入但 `~/.agent-sessions/<專案>/latest.md` 未更新 → 擋一次並提示喚醒 handoff-verifier | 中；需「僅擋一次」防死循環 |
...
| 3 | 接線 | — | 專案 `.claude/settings.json` 掛 Stop hook（PreToolUse 不動，已有全域 cost-aware-approval 承擔）；`inject.sh` 增 `install_stop_hook()`：下游 settings.json **指向正本腳本、不複製**（實作時偏離原稿「複製」方案——複製即產生會漂移的下游副本，2026-07-07 lessons 教訓） | 極低；下游零副本，正本改動即全域生效 |
